...
class Button:
    ...
    @staticmethod
    def pre_init():
        Button.FONT = pygame.font.SysFont("sans", 28)
        Button.FONT_CACHE = {char: Button.FONT.render(char, False, c.WHITE) for char in c.CHARS}
        Button.FONT_CACHE_DISABLED = {char: Button.FONT.render(char, False, (50, 50, 50)) for char in c.CHARS}
        Button.FONT_CACHE_HOVER = {char: Button.FONT.render(char, False, c.YELLOW) for char in c.CHARS}
    ...
    def is_hovered(self):
        ...
        mpos = (
            mpos[0] * c.WINDOW_WIDTH/c.SCALED_WINDOW_WIDTH,
            mpos[1] * c.WINDOW_HEIGHT/c.SCALED_WINDOW_HEIGHT
        )
